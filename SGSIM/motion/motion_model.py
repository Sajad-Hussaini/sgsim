...
class Motion:
    ...
    def set_range(self, option: str, range_slice: tuple[float, float]|np.ndarray):
        ...
        self.t = np.round(self.t[mask] - self.t[mask][0], 3)
    ...
    def save_simulations(self, filename: str, option: tuple[str, ...] = ('spectra', 'motions', 'peak_motions', 'characteristics')):
        """
        Save simulation data to an HDF5 file with improved structure.

        filename: The name of the HDF5 file to save the data to.

        option: A tuple of strings to specify the data to save (e.g., 'spectra', 'motions', 'peak_motions', 'characteristics')
            spectra: Save Fourier Amplitude and Response Spectra using 5% damping ratio
            motions: Save time series of ground motions
            peak_motions: Save peak ground motion parameters
            characteristics: Save ground motion characteristics including cumulative energy, zero crossing etc
        """
        option = map(str.lower, option)
        with h5py.File(filename, 'w') as hdf:
            if 'spectra' in option:
    ...
    @classmethod
    def from_simulations(cls, filename: str, option: tuple[str, ...] = ('spectra', 'peak_motions', 'characteristics')):
        """
        Load simulation data from an HDF5 file and create a Motion instance.
        Note that existence of 'motions' group including npts, dt, ac, vel, disp datasets is required.

        filename: The name of the HDF5 file to load the data from.

        option: A tuple of strings to specify the data to load (e.g., 'spectra', 'motions', 'peak_motions', 'characteristics')
            spectra: load Fourier Amplitude and Response Spectra using 5% damping ratio
            peak_motions: load peak ground motion parameters
            characteristics: load ground motion characteristics including cumulative energy, zero crossing etc
        """
        option = map(str.lower, option)
        with h5py.File(filename, 'r') as hdf:
            npts = hdf['motions/npts'][:]
            dt = hdf['motions/dt'][:]
            t = hdf['motions/t'][:]
            ac = hdf['motions/ac'][:]
            vel = hdf['motions/vel'][:]
        ...
        motion_instance = cls(npts=npts, dt=dt, ac=ac, vel=vel, disp=disp)
        motion_instance._t = t
        if 'spectra' in option:
    ...
    def from_file(cls, file_path: str | tuple[str, str], source: str, **kwargs):
        """
        Read ground motion data from a file
    ...
    def from_model(cls, model):
        """ Read ground motion data from a stochastic model """
